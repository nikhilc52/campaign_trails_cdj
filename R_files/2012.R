...
democrat <- read_csv('obama_2012.csv')
...
##################
republican <- read_csv('romney_2012.csv')
...
animation <- ggplot() +
  ...
  annotate("text", label="Romney",x=-27, y=23)+
  theme_void()+
  labs(title="2012 Presidental Campaign Trail", 
       subtitle="{format(frame_time, \"%h. %d\")}",
       caption="<br><br>Nikhil Chinchalkar for Cornell Data Journal | p2012.org | 2024 ")+
